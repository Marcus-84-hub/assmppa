...
const Step1Identification: React.FC<Props> = ({ data, onUpdate, onNext }) => {
  ...
  const handleSchoolSelect = (school: Partial<SchoolInfo>) => {
    onUpdate({
      ...data,
      ...school,
      // Ensure nested objects preserve existing values or defaults if needed
    });
  };
            ...
            <div className="md:col-span-9">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 rounded-xl focus:ring-primary-500 transition-all font-semibold"
                />
                <button
                  onClick={() => setIsModalOpen(true)}
                  className="bg-primary-500/10 hover:bg-primary-500 text-primary-600 hover:text-white px-4 py-2 rounded-xl transition-all flex items-center gap-2 font-bold whitespace-nowrap border border-primary-500/20"
                >
